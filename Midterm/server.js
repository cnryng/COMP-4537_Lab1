...
app.get('/pokemonsAdvancedFiltering', (req, res) => {
    ...
    if (req.query.comparisonOperators) {
        const comparisonsArray = req.query.comparisonOperators.split(",");
        console.log(comparisonsArray);
        comparisonsArray.forEach(element => {
...
app.patch("/pokemonsAdvancedUpdate", (req, res)=>{
    ...
    if (req.query.pushOperator) {
        const sortArray = req.query.pushOperator.split(",");
        sortArray.forEach(element => {
            element = element.trim();
            element = element.replace("[", "");
            element = element.replace("]", "");
            patchSelect["$push"]["type"].push(element);
        })
    }
    console.log(patchSelect);

    let responseObject = {}

    pokemonModel.updateMany(parsedQueryParams, patchSelect).exec((err, doc) => {
        if (err) {
            res.json(getMongooseErrorMessage(err, doc));
        } else {
            pokemonModel.find(parsedQueryParams, (err, doc) => {
                if (err) {
                    responseObject["msg"] = getMongooseErrorMessage(err, doc);
                } else {
                    responseObject["msg"] = "Updated Successfully";
                    responseObject["pokeInfo"] = doc;
                }
                res.json(responseObject);
            })
        }
